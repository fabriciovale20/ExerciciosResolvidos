...
def valorPagamento(prestacao, atraso):
    if atraso > 0:
        prestacao += prestacao * (0.03 + (atraso * 0.001))
    
    return prestacao

total_pago = qnt_prestacao = 0
while True:
    valor_prestacao = float(input('Valor da prestação R$ '))
    if valor_prestacao == 0:
        break

    numero_dias_atraso = int(input('Dias em atraso: '))

    prestacao = valorPagamento(valor_prestacao, numero_dias_atraso)
    qnt_prestacao += 1
    total_pago += prestacao

    print(f'Prestação de R$ {prestacao:.2f}.')

print(f'Total de prestações pagas {qnt_prestacao}, valor total pago R$ {total_pago:.2f}.')
